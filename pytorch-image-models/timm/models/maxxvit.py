...
class AttentionCl(nn.Module):
    ...
    def forward(self, x, shared_rel_pos: Optional[torch.Tensor] = None):
        ...
        if verbose_debug: print('AttentionCL forward, self.dim_head', self.dim_head, ',self.num_heads', self.num_heads, ', x.shape', x.shape)

        B = x.shape[0]
        restore_shape = x.shape[:-1]
        if verbose_debug: print('head_first', self.head_first)
        if self.head_first:
            q, k, v = self.qkv(x).view(B, -1, self.num_heads, self.dim_head * 3).transpose(1, 2).chunk(3, dim=3)
        else:
            q, k, v = self.qkv(x).reshape(B, -1, 3, self.num_heads, self.dim_head).transpose(1, 3).unbind(2)
        if verbose_debug: print(f'q: {q.shape}, k: {k.shape}, v: {v.shape}')
        # shape q: torch.Size([19800, 2, 1, 32])
        # 19800 = 195 x 50 x 2 (batch size): every element in batch is flattened here and gets
        # projected to dim_head dimension with num_heads heads.
        # 2: number of heads
        # 1: flattened
        # 32: dim_head

        if verbose_debug: print('self.fused_attn', self.fused_attn)
        if self.fused_attn:
            attn_bias = None
            if self.rel_pos is not None:
                attn_bias = self.rel_pos.get_bias()
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        if verbose_debug: print('AttentionCL forward end', x.shape)
        return x
...
class MbConvBlock(nn.Module):
    ...
    def forward(self, x):
        # print('MbConvBlock input', x.shape)
        shortcut = self.shortcut(x)
        # print('MbConvBlock shortcut', shortcut.shape)
        ...
        # print('MbConvBlock final', x.shape)
        return x
...
class PartitionAttentionCl(nn.Module):
    ...
    def _partition_attn(self, x):
        ...
        # print('_partition_attn', 'block: ', self.partition_block, 'x.shape', x.shape, 'partition_size', self.partition_size)
        ...
        # print('partitioned shape', partitioned.shape)
    ...
    def forward(self, x):
        if verbose_debug: print('PartitionAttentionCl forward')
...
class PartitionAttention2d(nn.Module):
    ...
    def forward(self, x):
        if verbose_debug: print('blub')
        x = x + self.drop_path1(self.ls1(self._partition_attn(self.norm1(x))))
        if verbose_debug: print('blab')
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        return x
...
class MaxxVitBlock(nn.Module):
    ...
    def __init__(
        ...
        if verbose_debug: print('MaxxVitBlock attn_kwargs', attn_kwargs)
    ...
    def forward(self, x):
        # NCHW format
        # if verbose_debug: 
        # print('MaxxVitBlock forward')
        x = self.conv(x)
        # print('MaxxVitBlock forward, after conv:', x.shape)
        ...
        # print('MaxxVitBlock forward, after attention:', x.shape)
        return x
...
class MaxxVitStage(nn.Module):
    def __init__(
        ...
        blocks = []
        if verbose_debug: print('block_types', block_types)
        for i, t in enumerate(block_types):
            ...
            if t == 'C':
                ...
                if verbose_debug: print(f'MaxxVitStage, i={i}, t={t}')
                if verbose_debug: print(in_chs, out_chs, f'stride: {block_stride}')
                blocks += [MaxxVitBlock(
                    in_chs,
                    out_chs,
                    stride=block_stride,
    ...
    def forward(self, x):
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if verbose_debug: print('MaxxVitStage forward')
            x = self.blocks(x)
        return x
