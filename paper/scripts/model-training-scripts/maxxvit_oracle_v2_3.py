...
from philosophers_stone._vendor.timm.models._registry import generate_default_cfgs, register_model
from philosophers_stone._vendor.timm.models._manipulate import named_apply, checkpoint_seq
from philosophers_stone._vendor.timm.models._features_fx import register_notrace_function
from philosophers_stone._vendor.timm.models._builder import build_model_with_cfg
from philosophers_stone._vendor.timm.layers import RelPosMlp, RelPosBias, RelPosBiasTf, use_fused_attn
from philosophers_stone._vendor.timm.layers import trunc_normal_tf_, to_2tuple, extend_tuple, make_divisible, _assert
from philosophers_stone._vendor.timm.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv2d, create_pool2d
from philosophers_stone._vendor.timm.layers import Mlp, ConvMlp, DropPath, LayerNorm, ClassifierHead, NormMlpClassifierHead, NormMlpClassifierHead2
from philosophers_stone._vendor.timm.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
...
class Stem(nn.Module):
    ...
    def __init__(
        ...
        # self.norm4 = norm_act_layer(128)
        # self.conv5 = Conv2d(128, 256, kernel_size=(3, 2), stride=(2, 2))
        # self.norm5 = norm_act_layer(256)
        # self.conv6 = Conv2d(256, 512, kernel_size=(3, 3), stride=(2, 2))
        # self.norm6 = norm_act_layer(512)
        # self.conv7 = Conv2d(512, 1024, kernel_size=(3, 3), stride=(2, 2))
        # self.norm7 = norm_act_layer(1024)
        # self.conv8 = Conv2d(1024, 2048, kernel_size=(3, 1), stride=(2, 1))
        # self.norm8 = norm_act_layer(2048)
...
class MaxxVit_oracle_v2_3(BaseOracleModule):
    ...
    def __init__(
        ...
        self.stem = Stem(
            in_chs=in_chans,
            out_chs=embed_dim, # cfg.stem_width,
        ...
        for i in range(num_stages):
            if i in [0]:
                ...
                stage_stride = 2
            if i == 0:
                transformer_cfg.window_size = (103, 24) # #(6, 24)
                transformer_cfg.grid_size = (103, 24) # (12, 12)
            elif i == 1:
                transformer_cfg.window_size = (103, 12) # (6, 12)
                transformer_cfg.grid_size =(103, 12) # (6, 12)
            elif i == 2:
        ...
        # self.head = NormMlpClassifierHead2(
